...
class feVar2d:
    """
    generate P, T, Pb, Tb
    have method that can generate boudnaryedge matrix, and boundary node matrix
    only useful for uniform rectangle mesh !!!
    """
    def __init__(self,  xmin, xmax, ymin, ymax, nx, ny, basisType):
        ...
        :param basisType: basis function type, 
            201-> linear basis function
            202-> quadratic basis function
        :type basisType: int
        """
        self.xmin = xmin
        self.xmax = xmax
        ...
        self.nmy = self.ny+1   # mesh nodes nubmer in y dir
        ...
        if self.bt == 201:
            ...
            self.Nb = self.nbx*self.nby  # FE node number
            self.hbx = self.hx
            self.hby = self.hy
    ...
    def getMatP(self):
        ...
        ref: ppt ch2 p12, p8
        """
        Nmx = self.nmx  # mesh nodes nubmer in x dir
        Nmy = self.nmy  # mesh nodes number in y dir
    ...
    def getMatBCedgeAndNode(self, ltype=-1, rtype=-1, btype=-1, ttype=-1):
        ...
        # left boundary, cover top boundary
    ...
    def locBasis(self, x, y, vertMat, bIdx, dIdx_x, dIdx_y):
        ...
        :param vertMat: vertice coordinate, 2*3 matrix
                vertMat[0,0] = x1  # relate to (0,0) in reference frame
                vertMat[1,0] = y1
...
class Solver2d(feVar2d):
    ...
    def getVecb(self, fFunc, pd, qd, t=0):
        """
        ref: ppt ch3 p51
        ...
        return sopt._genVecb(fFunc, self.Nb,self.N, self.matP, self.matT, self.matTb,
                            self.bt, self.beta, pd,qd, self.cRefGaussMat, self.pRefGaussMat, t
                            )

    def treatDirichletBC(self, gFunc, matbcNode, matA, vecb,t):
        """
        ref: ppt ch3 p57
